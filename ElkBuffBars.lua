...
if LE_EXPANSION_LEVEL_CURRENT < (LE_EXPANSION_MISTS_OF_PANDARIA or 4) then -- @todo remove hardcoded value once in Classic
...
function ElkBuffBars:OnInitialize()
    ...
    if not self.db.global.build or self.db.global.build ~= build then
...
function ElkBuffBars:HandleFrame_Blizzard_BuffFrame(hide)
    ...
    elseif C_EditMode then
        -- newer WoW versions
        if hide then
            if hidden_blizzard_frames["AuraFrame"] == nil then
                BuffFrame:HookScript("OnShow", function() if self.db.profile.hidebuffframe then BuffFrame:Hide() end end)
                DebuffFrame:HookScript("OnShow", function() if self.db.profile.hidebuffframe then DebuffFrame:Hide() end end)
            end
            BuffFrame:Hide()
            DebuffFrame:Hide()
            hidden_blizzard_frames["AuraFrame"] = true
        elseif hidden_blizzard_frames["AuraFrame"] then
            BuffFrame:Show()
            DebuffFrame:Show()
            hidden_blizzard_frames["AuraFrame"] = false
        end
    else
        -- newer WoW versions
        if hide then
        ...
        elseif hidden_blizzard_frames["AuraFrame"] then
            ...
            DebuffFrame:Show()
            DebuffFrame:Update()
            hidden_blizzard_frames["AuraFrame"] = nil
        end
    end
end

function ElkBuffBars:HandleFrame_Blizzard_TemporaryEnchantFrame(hide)
    if not TemporaryEnchantFrame then
        -- newer WoW versions show them using regular aura frames
        return
    end
    if hide then
...
function ElkBuffBars:HandleFrame_Blizzard_MiniMapTracking(hide)
    ...
    if MinimapCluster and MinimapCluster.Tracking then
        if MinimapCluster.Tracking.Button.intrinsic == "DropdownButton" then
    ...
    elseif MiniMapTrackingButton then
        if MiniMapTrackingButton.intrinsic == "DropdownButton" then
            -- hiding the button currently prevents showing the selection menu as menues for invisible buttons are closed via onUpdate
            return
        end
        if hide then
        ...
        -- classic version with just one tracking
        if hide then
...
function ElkBuffBars:GetSAB()
    if #cache_SAB > 0 then
        ...
        if WOW_PROJECT_ID == WOW_PROJECT_MAINLINE then -- @todo check
...
--[===[@debug@
            if C_UnitAuras.GetAuraDataByAuraInstanceID(unit, aura.auraInstanceID) == nil then
                self:Print("broken auraInstanceID", aura.auraInstanceID)
            end
--@end-debug@]===]
            ...
            local canStealOrPurge = aura.isStealable
    ...
    function ElkBuffBars:ScanData_UnitAura(unit, auratype)
        ...
        while true do
            ...
            dt.realtype         = auratype
            dt.debufftype       = debuffType
            dt.expires          = expirationTime and expirationTime > 0
            dt.expirytime       = expirationTime
            dt.timemax          = duration or 0
            ...
            general = {
                ...
                args = {
                    ...
                    tenchframe = {
                        ...
                        disabled = not TemporaryEnchantFrame,
                        get = function(info)
                            if not TemporaryEnchantFrame then
                                return false
                            end
                            return ElkBuffBars.db.profile.hidetenchframe
                        end,
                        set = function(info, v)
                    ...
                    trackingframe = {
                        ...
                        disabled = (MiniMapTracking and MiniMapTrackingButton and MiniMapTrackingButton.intrinsic == "DropdownButton")
                            or (MinimapCluster and MinimapCluster.Tracking and MinimapCluster.Tracking.Button.intrinsic == "DropdownButton"),
                        get = function(info)
                            if
                                (MiniMapTracking and MiniMapTrackingButton and MiniMapTrackingButton.intrinsic == "DropdownButton")
                                or (MinimapCluster and MinimapCluster.Tracking and MinimapCluster.Tracking.Button.intrinsic == "DropdownButton")
                            then
                                return false
                            end
