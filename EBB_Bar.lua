...
--~ 		ElkBuffBars:Print(string.format("|cffff0000[EBB] ERROR:|r (ElkBuffBars.Bar:OnUpdate) %d - %s - %f / %f", data.index, data.name, self.timeleft, data.timemax))
--~ 		fraction = 0
--~ 	end
...
--~ 			if fraction > 1.001 then
--~ 				ElkBuffBars:Print(string.format("|cffff0000[EBB] ERROR:|r (ElkBuffBars.Bar:UpdateData) %d - %s - %f / %f", data.index, data.name, self.timeleft, data.timemax))
--~ 				fraction = 0
--~ 			end
    ...
    if data.expires and data.realtype ~= "FAKE" then -- no scripts for Blessing of Demonstration
    ...
    if not InCombatLockdown() then
        if not layout.clickthrough and playerunit[self.parent.layout.target]
          and (data.realtype == "BUFF" or data.realtype == "TENCH" or data.realtype == "TRACKING") then
            local SAB = self.SAB
            if not SAB then
